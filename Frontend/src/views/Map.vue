<template>
  <div class="map-container">
    <search @change="getSearchValue($event)" @city-change="cityChange()" />
...
export default {
  ...
  methods: {
    ...
    executeQuery: async function (value) {
      this.polylines = []
      // if city was changed, get new city center
      if (this.cityChanged && value.city !== null && value.timestamp !== null) {
        console.log('City changed to: ' + value.city)
        await this.getCenter(value)
        this.cityChanged = false
      }
      // get incidents, compare, and pass coordinates
      if (value.city !== null && value.timestamp !== null) {
        ...
        console.log(
          'Comparison incidents received: ' + this.comparisonData.length
        )
